...
from typing import Any, TYPE_CHECKING, Iterable, Type
...
class HomeworksHub(Hub):
    def __init__(self, bus: EventBus, homeworks_config: dict[str, Any]) -> None:
        ...
        self._coordinator = ConnectionCoordinator(self._client, self._handle_response)
    ...
    def _notify_state_update(self, state: ConnectionState):
        if self._connection_state_listener != None:
            ...
            self._connection_state_listener.on_connection_state_changed(old_state=old_state, new_state=state)
    ...
    def send_command(self, name: str, *args: str):
        if len(args) > 0:
            ...
            data = name
        self._coordinator.send_request(RequestMessage(RequestMessageKind.SEND_COMMAND, data))
